...
public class LotofacilDezenasMaisSorteadas extends Activity{
	protected static final String CATEGORIA = "loterias";
	public static LotofacilMaisSorteadasRepositorio repositorio;
	public static LotofacilRepositorio lr;
	private List<LotofacilMaisSorteadas> listaLotofacilMaisSorteadas;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		lr = new LotofacilRepositorio(this);
		repositorio = new LotofacilMaisSorteadasRepositorio(this);
		if(lr.getCursor().getCount() > 0){
        ...
        btListar.setOnClickListener(new OnClickListener() {
			public void onClick(View view) {
				...
				if(lr.getCursor().getCount() > 0){
					dialog = ProgressDialog.show(LotofacilDezenasMaisSorteadas.this, "", "Aguarde...",false,true);
					carregaDados(n);
				}	
			}
		});
        ...
        btGerarJogos.setOnClickListener(new OnClickListener() {
			public void onClick(View view) {
				if(repositorio.getDezenasSelecionadas().getCount()<15){
					Toast.makeText(LotofacilDezenasMaisSorteadas.this, "Selecione pelo menos 15 dezenas", Toast.LENGTH_SHORT).show();
				}else{
					gerarJogos();
				}
			}
	...
	protected void atualizaLista(){
		...
		listaLotofacilMaisSorteadas = repositorio.listarLotofacilMaisSorteadas();
		//Adaptador da lista customizada para cada linha de um partido
		LotofacilMaisSorteadasListAdapter adapter = new LotofacilMaisSorteadasListAdapter(this,listaLotofacilMaisSorteadas);
	...
	protected void gerarJogos(){
		...
		Intent it = new Intent(this, LotofacilGerarJogos.class);
		startActivity(it);
	}
	...
	private void carregaDados(final int concursos){
		new Thread(){
			@Override
			public void run(){
					repositorio.Deletar(LotofacilMaisSorteadasColunas._ID + ">?", new String[]{"0"});
