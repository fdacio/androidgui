...
/**
 * Activity que utiliza o TableLayout para editar o Presidente
 * 
 * @author fdacio
 * 
 */
public class EditarPresidente extends Activity {
	...
	protected void onCreate(Bundle icicle) {
		...
		setContentView(R.layout.form_editar_presidente);
		...
		if (extras != null) {
			id = extras.getLong(PresidenteColunas._ID);

			if (id != null) {
				// é uma edição, busca o Presidente...
				Presidente p = buscarPresidente(id);
				...
				campoNomeVice.setText(p.getNomeVicePresidente());
				campoFotoVice.setText(p.getFotoVicePresidente());
			}
		}
		...
		// Listener para pesquisar foto Presidente
		...
		Log.i(CATEGORIA,"Iniciou a Tela de Editar de Presidente");
	}
	...
	public void salvar() {
		...
		p.setNomeVicePresidente(campoNomeVice.getText().toString());
		p.setFotoVicePresidente(campoFotoVice.getText().toString());
		// Salvar
		salvarPresidente(p);
	...
	protected Presidente buscarPresidente(long id) {
		return CadastroPresidente.repositorio.buscarPresidente(id);
	}

	// Salvar o governador
	protected void salvarPresidente(Presidente p) {
		CadastroPresidente.repositorio.Salvar(p);
	}

	// Excluir o governador
	protected void excluirPresidente(long id) {
		CadastroPresidente.repositorio.Deletar(id);
	}
