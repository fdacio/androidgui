...
public class DeteccaoGestos extends Activity implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener{

    private final int SWIPE_MIN_DISTANCE = 100;
    private final int SWIPE_MIN_VELOCITY = 110;
    private final int FLIPPER_INTERVAL = 3000;
    private final int ANIMATION_DURATION = 1000;
    private final int VIBRATION = 200; 
	...
	flipper.setAnimation(animleftin);
	flipper.setAnimation(animleftout);
	flipper.setFlipInterval(FLIPPER_INTERVAL);
	...
	if(xvelocity > this.SWIPE_MIN_VELOCITY && xdiff > this.SWIPE_MIN_DISTANCE){
		...
		flipper.setInAnimation(animrightout);
	    }
		...
		flipper.setInAnimation(animupout);
	    }
	    else{
		++currentview;
		if(currentview >= views.size()){
		...
		flipper.setInAnimation(animdownout);
	    }
	    flipper.scrollTo(0, 0);
	    flipper.setDisplayedChild(currentview);
	}
